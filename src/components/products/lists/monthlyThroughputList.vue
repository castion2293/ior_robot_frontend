...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            items () {
                let date_ok = _.map(this.Monthly_Throughput.items_ok, item => {
                    return item.date
                })

                let date_ng = _.map(this.Monthly_Throughput.items_ng, item => {
                    return item.date
                })
                ...
                let throughput_group =  _.map(dates, date =>{
                    return {
                        value: false,
                        date: new Date(date),
                        ok: this.findOKNumber(this.Monthly_Throughput.items_ok, date),
                        ng: this.findNGNumber(this.Monthly_Throughput.items_ng, date),
                    }
                })
        ...
        methods: {
            ...
            findNGNumber (items, date) {
                let foo = _.find(this.Monthly_Throughput.items_ng, function (ng) {
                        return ng.date === date
                    })
